      ...
      chrome.debugger.onEvent.addListener((source, method, params) => {
        if (method === "Overlay.nodeHighlightRequested") {
          chrome.debugger.sendCommand(
            ...
            "Runtime.evaluate",
            {
              expression: `document.elementFromPoint(${params.x}, ${params.y}).outerHTML`,
            },
            (result) => {
              ...
              navigator.clipboard.writeText(selector).then(() => {
                alert(`Debug Selector copied: ${selector}`);
              });
...
function generateCypressSelectorFromHTML(elementHTML) {
  const element = new DOMParser().parseFromString(elementHTML, "text/html")
    .body.firstChild;
